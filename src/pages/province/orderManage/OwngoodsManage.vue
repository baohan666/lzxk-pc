		...
		<div id="" v-show="!handleEditShow">
			<div class="search_wrap">
				<el-form :inline="true" class="demo-form-inline">
					<el-form-item label="提交人：">
						<el-input v-model="oweName" placeholder="请输入提交人"></el-input>
					</el-form-item>
				...
				<el-form ref="form" :inline="true" class="demo-form-inline slideDown">
					<el-form-item label="所属区域：">
						<el-input v-model="treeName" placeholder="请输入省区名称"></el-input>
					</el-form-item>
			...
			<div class='table_wrap'>
				<el-table :data="tableData" border>
					...
					<el-table-column prop="oweName" label="提交人" align='center'>
					</el-table-column>
					<el-table-column prop="position" label="提交人职级" align='center'>
					</el-table-column>
					<el-table-column prop="treeName" label="所属区域" align='center'>
					</el-table-column>
					...
					<el-table-column prop="date" label="操作" align='center' min-width='150px'>
						<template slot-scope="scope">
							<div class="operateBtn" v-if="scope.row.oweStatus == 1&&scope.row.isRemindStr == '否'" @click="dialogVisible = true,id = scope.row.id,dailogCon='确定提醒省区可以提单?',dailogConShow = true,dailogTitleType='提示' ">提醒</div>
							<div class="operateBtn" v-if="scope.row.oweStatus == 1" @click="dialogVisible = true,id = scope.row.id,dailogCon='确定提醒地县可以提单?',dailogConShow = false,dailogTitleType='提示'">不许提单</div>
...
-->
			<el-dialog :title="dailogTitleType" :visible.sync="dialogVisible" :append-to-body='true' width="30%" class='elDialogAddress'>
				<div class="dailogCon" v-if="dailogConShow" style="text-align: center;">
					<span>{{dailogCon}}</span>
				</div>
				...
				<span slot="footer">
					<span v-if="!dailogConShow" @click="notAllow()" class="dialogBtnTrue">确定</span>
					<span v-if="dailogConShow && dailogTitleType!='不许提单原因'" @click="remindProvince()" class="dialogBtnTrue">确定</span>
				</span>
			</el-dialog>
...
<script>
	...
	export default {
		...
		methods: {
			getData(pageIndex, pageSize, oweName, treeName, position, oweStatus, startCreateDate, endCreateDate, isRemind) {
				getSQOweList({
					'pageSize': pageSize //每页大小
						,
					'pageIndex': pageIndex,
					'oweName': oweName //事业部id  1.心脑事业部 2.风湿及骨科事业部3.云事业部4.消化事业部
						,
					'treeName': treeName //省区名字
						,
					'position': position //省区经理名字
						,
			...
			handleCheck(index, row) {
				this.$router.push({path:'saleOrderDetailSQ',query:{id:row.oldOrdersId}});
			}
