        ...
        <div class='qualifiInfo_table'>
          ...
          <div v-if="searchCity=='' && searchCounty==''" v-for = '(item,index) in tableData' :key="index">
            <el-row>
              ...
              <el-col :span="5">
                <input
                  ...
                  placeholder="请输入价格"
                  @change="handleChange(item.cityObject,'areaPrice')"
                  type="number"
              ...
              <el-col :span="4"><span class="operateBtn" @click="showChangePriceLog(item.cityObject.id, item.cityObject.managerUserId, item.cityObject.position,  item.cityObject.userId)">改价日志</span></el-col>
            </el-row>
            ...
            <el-row v-for = '(menuCounty,index) in item.countyArray' :key="index">
              ...
              <el-col :span="5">
                <input v-model.trim="menuCounty.areaPrice"
                       placeholder="请输入价格"
                       @change="handleChange(menuCounty,'countyPrice')"
                       type="number"
                       @keydown="handleInput2"
                       :class="{'tableInput': true, 'color_red' : infoList.countyStandardPrice < menuCounty.areaPrice, 'color_blue' : infoList.countyStandardPrice > menuCounty.areaPrice}">
              </el-col>
              <el-col :span="4">
                <span class="operateBtn"
                      @click="showChangePriceLog(menuCounty.id, menuCounty.managerUserId, menuCounty.position, menuCounty.userId)">
                  改价日志
                </span>
              </el-col>
            </el-row>
          ...
          <div v-if="searchCity!='' || searchCounty!=''" v-for = '(item,index) in tableData' :key="index">
            <el-row>
              <el-col :span="5">{{item.cityObject.areaName}}22</el-col>
              <el-col :span="5">{{item.cityObject.userName}}</el-col>
              <el-col :span="5">{{item.cityObject.position}}</el-col>
              <el-col :span="5">
                <input
                  ...
                  :class="{'tableInput': true, 'color_red' : infoList.cityStandardPrice < item.cityObject.areaPrice, 'color_blue' : infoList.cityStandardPrice > item.cityObject.areaPrice}">
              </el-col>
              <el-col :span="4">
                <span class="operateBtn"
                      @click="showChangePriceLog(item.cityObject.id, item.cityObject.managerUserId, item.cityObject.position, item.cityObject.userId)">改价日志</span>
              </el-col>
            </el-row>
            <el-row v-for = '(menuCounty,index) in item.countyArray' :key="index">
              <el-col :span="5">{{menuCounty.areaName}}</el-col>
...
<script>
  ...
  import {getStandardPriceData, getChangePriceLogData, saveBatchPartChaPriceData} from 'src/api/province_url/goodsManage';
  ...
  export default {
    ...
    data() {
      return {
        ...
        // searchCity: '',//地区经理及地区名称
        // searchCounty: '',//县区经理及县区名称
        changeReason: '',//改价原因
    ...
    methods: {
      initData(){
        getStandardPriceData({
          goodsId: this.$route.query.goodsId,
          managerId:this.$route.query.userId,
          userId:this.$route.query.userId,
          searchCity:this.goodsName,
          searchCounty:this.goodsName_xian
        }).then(res => {
