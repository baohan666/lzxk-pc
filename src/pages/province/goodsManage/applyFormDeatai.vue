<template>
	<div>
		...
		<div class="operateWraper" v-show="appStatus==1">
			<div>
				<div class="div-inline el-button-red" @click = "checkApply()">撤回</div>
			</div>
		</div>
...
<script>
  ...
  import {getApplyFormDetailData, checkApplyPassData, approveApplyPassData} from 'src/api/province_url/goodsManage';
  ...
  methods: {
    initData(){
      getApplyFormDetailData({applyId: this.$route.query.applyId, pageSize: 10, pageIndex:1 }).then(res => {
        ...
        this.infoList=datas.headObj; // 头部信息
        this.tableData =datas.list.dataList; // 表格数组
        this.totalRecords=datas.list.totalRecords; // 总条数
        this.totalPageCnt=datas.list.totalPageCnt; // 总页数
        this.pageIndex=datas.list.pageIndex; // 当前页
        this.appStatus = datas.headObj.appStatus; // 审核状态
      })
    },
		//确定按钮
		checkSure(){
			let datalist = {
				applyId: this.$route.query.applyId, // 申请单id
				userId: getStore('accountId'), // 处理人id
				reason: this.changeReason // 撤回原因
			};
      checkApplyPassData(datalist).then( res => {
        this.$message.success("处理成功");
        this.dialogReasonVisible = false;
        this.$router.push({"path": "applicationFormSQ"});
      })
		...
		//审核通过或者不通过
		checkApply(){
      this.dialogReasonVisible=true; // 显示弹窗
      this.dialogTitle='撤回原因'; // 弹窗 标题
      this.changeReason="" // 撤回原因 内容
