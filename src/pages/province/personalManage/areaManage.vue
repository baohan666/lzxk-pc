	...
	<div class="accountManage">
		<div id="" v-show="!handleEditShow">
			<div class="search_wrap">
				<el-form :inline="true" class="demo-form-inline">
					...
					<!--<div class="province_city_county el-form-item">
				...
				<el-table :data="tableData" border>
					...
					<el-table-column prop="date" label="操作" align='center'>
						<template slot-scope="scope">
							<div class="operateBtn edit" v-if='scope.row.relevanceStatusStr == "已关联"' @click="handleRevision(scope.$index, scope.row )	,treeNameTitle=scope.row.provinceName,nameTitle=scope.row.name	">修改</div>
							<div class="operateBtn" v-if='scope.row.relevanceStatusStr == "已关联"' @click="handleCheck(scope.$index, scope.row ),treeNameTitle=scope.row.provinceName,nameTitle=scope.row.name	">查看</div>
							<div class="operateBtn" v-if='scope.row.relevanceStatusStr != "已关联"' @click="handleRelevance(scope.$index, scope.row)">关联</div>
						</template>
					</el-table-column>
				</el-table>
			</div>
		...
		<div v-show="handleEditShow" class="handleEditShow">
			...
			<div class="adminDetailBox">
				<span class="adminDetailTitle">{{handleTitle}}- <span>{{treeNameTitle}}-{{nameTitle}}</span> </span>
	
				<!--关联时-->
				<el-tree 
							v-show='treeShow && !revisionTreeShow'	
							v-loading="loading"	
							ref="tree"
							:data="dataTree" 
							:load="loadNode" 
...
<script>
	...
	} from 'src/api/headquarter_url/areaManage';
	...
	export default {
		data() {
			return {
				...
				provinceNameTitle:'',// 修改查看关联时提示提示
				nameTitle:'',// 修改查看关联时提示提示
				treeShow:true,
				...
				provinceName: '',
				disabledShow: false,
				handleEditShow: false, //查看修改关联显示隐藏
		...
		methods: {
			...
			getData(pageIndex, pageSize, relevanceStatus, name,  position) {
				getList({
			...
			updataPage(val){
				...
				this.getData(this.pageIndex, this.pageSize, this.selStatus, this.name, this.position)
			},
			//关联
			...
			        if(a=="childArea" && this.isArray(obj[a])){			
							for(let item of obj[a]){
								if(item.isChoose == 1 && item.areaLevel == 3 && this.handleTitle == '县区经理关联行政区'){
									this.resourceCheckedKey.push(item.areaCode)
								}								
								else if(item.isChoose == 1 && item.areaLevel == 3 && this.handleTitle == '地区经理关联行政区') {
									this.resourceCheckedKey.push(item.areaCode)
								}
							}	
			...
			search() {
				this.getData(1, this.pageSize, this.selStatus, this.name, this.position)
			},
		}
	}
</script>
