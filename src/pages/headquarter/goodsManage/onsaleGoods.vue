...
<template>
  <div>
    ...
    <div class="table_wrap">
      <button class="div-inline el-button-red bitchRemove " @click="batchRemove">批量下架</button>
      ...
      <el-table ref="multipleTable" :data="tableData3" border tooltip-effect="dark"
        ...
        <el-table-column prop="address" label="操作">
          <template slot-scope="scope">
            <span class="operateBtn " @click="goodsRemove(scope.row.id)">下架</span>
            <span class="operateBtn " @click="view(scope.row.id,1)">查看</span>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination :pageIndex='pageIndex' :pageSize='pageSize' :totalRecords='totalRecords'
                     :totalPageCnt='totalPageCnt' :pagin_show='pagin_show' @clickpage="updataPage"></el-pagination>
    </div>
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!--<button v-if="sureBtn_status==1" class="el-button-red" @click="removeSure">确定</button>
        <button v-else-if="sureBtn_status==2" class="el-button-red" @click="dialogVisible = false">确定</button>-->
      </div>
    </el-dialog>
...
<script>
  ...
  import {getStore, setDepartment, getAliOssUrl} from 'src/config/cache'
  import {
    getOnsalegoodsData, getCategoryListData, getManufacturData, getdrugTypeData,
    exportData, getUpateGdData, getGoodDetaiData
    ...
    methods: {
      ...
      initData(datalist) {
        ...
        getOnsalegoodsData(datalist).then(res => {
          ...
          console.log(datas.totalRecords);
      ...
      //获取生产企业
      getManufacturer() {
        if (Object.keys(this.manufacturerMenu).length > 0) {
          return false;
        }
      ...
      //获取剂型
      getDrugType() {
        if (Object.keys(this.drugTypeMenu).length > 0) {
          return false;
        }
...
//					this.sureBtn_status = 1;
          this.textTip = "您确定要下架这些商品吗？";
          let ids = this.tId.substring(0, this.tId.lastIndexOf(','));
          this.removeGoodss = {"ids": ids, status: 2};
...
//					this.sureBtn_status = 2;
      ...
      //单个下架
      goodsRemove($id) {
        this.removeGoodss = {"ids": $id, status: 2};
//				this.sureBtn_status = 1;
        this.textTip = "您确定要下架此商品吗？";
